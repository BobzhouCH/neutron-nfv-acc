...
DEVICE_OWNER_ROUTER_GW_INTERNAL = 'network:router_gateway_internal'
...
class L3_NAT_dbonly_mixin(l3.RouterPluginBase,
    ...
    def _create_router_gw_port(self, context, router, network_id, ext_ips):
        ...
        internal_gw_port = self._ensure_gw_internal_port(context, network_id)
        if not internal_gw_port:
            raise l3.UnableCreateGwInternalPort()

        ext_fip = ext_ips[0].get('floating_ip')
        gw_port_id = ext_fip.get("floating_port_id")
        floatingip_db = self._get_floatingip(context, ext_fip['id'])
        floatingip_db.update({'fixed_port_id': internal_gw_port['id'],
                              'router_id': router['id'],
                              'status': l3_constants.FLOATINGIP_STATUS_ACTIVE})
        port_db = self._core_plugin._get_port(context.elevated(), gw_port_id)
        port_db.update({'device_owner': DEVICE_OWNER_ROUTER_GW,
                        'device_id': router['id']})
        router_db = self._get_router(context, router['id'])
        router_db.update({'gw_port_id': gw_port_id})
        with context.session.begin(subtransactions=True):
    ...
    def _check_gw_internal_port(self, context):
        device_filter = {'device_owner': [DEVICE_OWNER_ROUTER_GW_INTERNAL]}
        return self._core_plugin.get_ports(context.elevated(),
                                                filters=device_filter)

    def _ensure_gw_internal_port(self, context, network_id):
        """Placeholder port to identify a floating ip which are used as gateway"""
        gw_internal_port = self._check_gw_internal_port(context)
        if not gw_internal_port:
            # all attribute are not used except 'device_owner'. network_id
            # is set to avoid exception
            gw_internal_port = self._core_plugin.create_port(context.elevated(), {
            'port': {'tenant_id': '',
                     'network_id': network_id,
                     'mac_address': '',
                     'fixed_ips': [],
                     'device_id': '',
                     'device_owner': DEVICE_OWNER_ROUTER_GW_INTERNAL,
                     'admin_state_up': True,
                     'status': 'ACTIVE',
                     'name': 'internal_gw'}})
            return gw_internal_port
        return gw_internal_port[0]

    def _validate_gw_info(self, context, gw_port, info, router, ext_ips):
        ...
        if network_id:
            ...
            if not network_db.external:
                ...
                raise n_exc.BadRequest(resource='router', msg=msg)
            ext_ips = self._ensure_ext_ip_is_floating_ip(context, ext_ips, network_id, router['tenant_id'])
            if not ext_ips:
                msg = _("Specified external ip %s is not a floating ip or "
                        "there's no remaining floating ip to use in external "
                        "network %s for tenant %s") % (ext_ips, network_id, router['tenant_id'])
                raise n_exc.BadRequest(resource='router', msg=msg)
        return network_id, ext_ips

    def _ensure_ext_ip_is_floating_ip(self, context, ext_ips, network_id, router_tenant_id):
        """Ensure IP on router use floating ip"""
        if ext_ips:
            if ext_ips and len(ext_ips) > 1:
            ...
            for ext_ip in ext_ips:
                fip_filter = {'floating_network_id': [network_id],
                              'tenant_id': [router_tenant_id],
                              'floating_ip_address': [ext_ip.get('ip_address')]}
                cons_fips = self.get_floatingips(context, fip_filter)
                for fip in cons_fips:
                    if fip.get('port_id', None):
                        msg = _("The specified IP has been used")
                        raise n_exc.BadRequest(resource='router', msg=msg)
                    ext_fips.append(fip)
        else:
            ...
            query = query.filter_by(floating_network_id=network_id,
                                    tenant_id=router_tenant_id,
                                    fixed_port_id=None)
            avaliable_fips = [p for p in query]
            ext_fips = []
            if avaliable_fips:
                ext_fips = avaliable_fips[:1]
        return [{"floating_ip": fip} for fip in ext_fips]

    def _delete_current_gw_port(self, context, router_id, router,
                                new_network_id):
        """Delete gw port if attached to an old network."""
        port_requires_deletion = router.gw_port
        if not port_requires_deletion:
            return
        admin_ctx = context.elevated()

        internal_gw_port = self._check_gw_internal_port(context)
        query = self._model_query(context, FloatingIP)
        query = query.filter_by(router_id=router_id)
        if internal_gw_port:
            query = query.filter(
                ~FloatingIP.fixed_port_id.in_([p['id'] for p in internal_gw_port]))
        if query.count():
        ...
        query = self._model_query(context, FloatingIP)
        floatingip_db = query.filter_by(floating_port_id=gw_port_id)[0]
        floatingip_db.update({'fixed_port_id': None,
                              'router_id': None,
                              'status': l3_constants.FLOATINGIP_STATUS_DOWN})
        port_db = self._core_plugin._get_port(context.elevated(), gw_port_id)
        port_db.update({'device_owner': DEVICE_OWNER_FLOATINGIP,
                        'device_id': floatingip_db['id']})
        router_db = self._get_router(context, router['id'])
        router_db.update({'gw_port_id': None})
        ...
        registry.notify(resources.ROUTER_GATEWAY,
                        ...
                        gateway_ips=floatingip_db['floating_ip_address'])
    ...
    def _create_gw_port(self, context, router_id, router, new_network_id,
                        ext_ips):
        new_valid_gw_port_attachment = ext_ips
    ...
    def _update_router_gw_info(self, context, router_id, info, router=None):
        ...
        network_id, ext_ips = self._validate_gw_info(context, gw_port, info,
                                                     router, ext_ips)
        ext_ip_change = self._check_for_external_ip_change(
            context, gw_port, ext_ips)
        if ext_ip_change or not ext_ips:
    ...
    def _check_for_external_ip_change(self, context, gw_port, ext_ips):
        ...
        gw_port_id = gw_port['id']
        new_gw_port_id = ext_ips[0].get('floating_ip')['id']
        return gw_port_id == new_gw_port_id
    ...
    def _confirm_router_interface_not_in_use(self, context, router_id,
        ...
        for fip_db in fip_qry.filter_by(router_id=router_id):
            if fip_db['fixed_ip_address'] and netaddr.IPAddress(fip_db['fixed_ip_address']) in subnet_cidr:
    ...
    def _check_and_get_fip_assoc(self, context, fip, floatingip_db):
        ...
        if fip.get('fixed_ip_address') and not fip.get('port_id'):
            ...
            raise n_exc.BadRequest(resource='floatingip', msg=msg)
        if fip.get('port_id'):
            if (floatingip_db.get('fixed_port_id', None) and
                self._is_internal_gw_port(context, floatingip_db['fixed_port_id'])):
                msg = _("The floating ip is used as router gateway. Thus cannot "
                        "be associate to a internal port")
                raise n_exc.BadRequest(resource='floatingip', msg=msg)
    ...
    def _update_fip_assoc(self, context, fip, floatingip_db, external_port):
        ...
        if 'bandwidth' in fip:
            update['bandwidth'] = fip['bandwidth']
    ...
    def _is_internal_gw_port(self, context, port_id):
        if not port_id:
            return
        device_filter = {'device_owner': [DEVICE_OWNER_ROUTER_GW_INTERNAL],
                         'id': [port_id]}
    ...
    def _delete_floatingip(self, context, id):
        ...
        internal_port = self._check_gw_internal_port(context)
        if internal_port:
            internal_port_ids = set(port['id'] for port in internal_port)
            if internal_port_ids and floatingip['fixed_port_id'] in internal_port_ids:
                msg = _("The floating ip to be deleted is used as router gateway.")
                raise n_exc.BadRequest(resource='router', msg=msg)
    ...
    def _build_routers_list(self, context, routers, gw_ports, gw_fips):
    ...
    def _get_sync_routers(self, context, router_ids=None, active=None):
        ...
        gw_ports = dict((r['gw_port']['id'], r['gw_port'])
                        ...
                        if r.get('gw_port'))
        gw_port_ids=[r['gw_port_id'] for r in router_dicts if r.get('gw_port_id')]
        gw_fips = self.get_sync_gw_floating_ips(context, gw_port_ids)
        return self._build_routers_list(context, router_dicts, gw_ports, gw_fips)

    def get_sync_gw_floating_ips(self, context, gw_port_ids):
        if not gw_port_ids:
            return []
        filters = {'floating_port_id': gw_port_ids}
        fips = self.get_floatingips(context, filters)
        for fip in fips:
            network = self._core_plugin.get_network(
                context, fip.get('floating_network_id', None))
            if network and provider.PHYSICAL_NETWORK in network:
                fip[provider.PHYSICAL_NETWORK] = network[provider.PHYSICAL_NETWORK]
        return fips
    ...
    def _get_sync_floating_ips(self, context, router_ids, gw_port_ids):
        ...
        # Filter out on gw_port_ids
        query = query.filter(FloatingIP.floating_port_id.notin_(gw_port_ids))
    ...
    def _get_router_info_list(self, context, router_ids=None, active=None,
        ...
        with context.session.begin(subtransactions=True):
            ...
            gw_port_ids = [router['gw_port_id'] for router in routers
                           if 'gw_port_id' in router and router['gw_port_id']]
            floating_ips = self._get_sync_floating_ips(context, router_ids, gw_port_ids)
