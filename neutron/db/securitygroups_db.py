...
security_group_default_rules = [
    cfg.StrOpt('security_group_default_rules_for',
               default='',
               help=_('Create security_group_default_rules for '
                      'all or default security group')),
    cfg.ListOpt('security_group_default_rules',
                default='',
                help=_('These security group rules will be added by default '
                       'when a security group is created.'
                       'Format is '
                       'ethertype,direction,protocol,port_range_min,port_range_max, '
                       'For example, allow ssh and icmp request: '
                       'security_group_default_rules = '
                       'ipv4:ingress:tcp:22:22,ipv4:ingress:icmp')),
]
cfg.CONF.register_opts(security_group_default_rules)
RULE_CONFIG_ATTRIBUTES_COUNT = 5
...
class SecurityGroupDbMixin(ext_sg.SecurityGroupPluginBase):
    ...
    def create_security_group(self, context, security_group, default_sg=False):
        ...
        with db_api.autonested_transaction(context.session):
            ...
            if (cfg.CONF.security_group_default_rules_for == 'all') or \
               (default_sg and (cfg.CONF.security_group_default_rules_for == 'default')):
                for default_rule in cfg.CONF.security_group_default_rules:
                    rule_info = default_rule.split(':')
                    if len(rule_info) > RULE_CONFIG_ATTRIBUTES_COUNT:
                        LOG.warning(_LW('Ignore invalid security group rule '
                                     '%(rule)s'), {'rule': rule_info})
                        continue
                    rule = {}
                    for idx, key in enumerate(('ethertype', 'direction', 'protocol',
                                               'port_range_min', 'port_range_max')):
                        try:
                            rule[key] = rule_info[idx]
                            if key == 'ethertype':
                                rule[key] = rule[key][:2].upper() + rule[key][2:]
                        except IndexError:
                            rule[key] = None
                    try:
    ...
    def _validate_port_range(self, rule):
        ...
        if ip_proto in [constants.PROTO_NUM_TCP, constants.PROTO_NUM_UDP]:
            ...
            for attr, field in [('port_range_min', 'type'),
                                ('port_range_max', 'code')]:
                if rule[attr] is not None and not (0 <= int(rule[attr]) <= 255):
