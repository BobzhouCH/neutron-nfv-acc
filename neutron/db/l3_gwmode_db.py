...
class L3_NAT_dbonly_mixin(l3_db.L3_NAT_dbonly_mixin):
    ...
    def _build_routers_list(self, context, routers, gw_ports, gw_fips):
        gw_port_id_gw_fips_dict = dict((fip['floating_port_id'], fip)
                                       for fip in gw_fips)
        for rtr in routers:
            ...
            if gw_port_id:
                ...
                rtr['gw_floating_ip'] = gw_port_id_gw_fips_dict[gw_port_id]
                if rtr['gw_floating_ip']:
                    rtr['gw_provider_physical_network'] = \
                        rtr['gw_floating_ip'].get(provider.PHYSICAL_NETWORK, None)
                # Add enable_snat key
