...
class NeutronDbPluginV2(db_base_plugin_common.DbBasePluginCommon,
    ...
    def create_network_db(self, context, network):
        ...
        with context.session.begin(subtransactions=True):
            #add date<2016.12.28>,issue:duplicate of network,subnet's name
            result = context.session.query(models_v2.Network).filter_by(tenant_id=tenant_id, name=n['name']).first()
            if result:
                err_msg = (_("The network '%(network_name)s' already exist") % {'network_name':n['name']})
                LOG.error(_LI(("The network '%(network_name)s' already exist") % {'network_name':n['name']}))
                raise n_exc.OverlappingOfName(msg=err_msg)
            #endif
    ...
    def update_network(self, context, id, network):
        ...
        with context.session.begin(subtransactions=True):
            ...
            #add date<2016.12.28>,issue:duplicate of network,subnet's name
            if n.get('name') and n.get('name') != network['name']:
                result = context.session.query(models_v2.Network).filter_by(tenant_id=network['tenant_id'], name=n['name']).first()
                if result:
                    err_msg = (_("The network '%(network_name)s' already exist") % {'network_name': n['name']})
                    LOG.error(_LI(("The network '%(network_name)s' already exist") % {'network_name': n['name']}))
                    raise n_exc.OverlappingOfName(msg=err_msg)
    ...
    def create_subnet(self, context, subnet):
        ...
        if subnetpool_id:
            result = context.session.query(models_v2.SubnetPool).filter_by(tenant_id=tenant_id, name=s['name']).first()
            if result:
                err_msg = (_("The subnetpool '%(subnet_name)s' already exist") % {'subnet_name': s['name']})
                LOG.error(_LI(("The subnetpool '%(subnet_name)s' already exist") % {'subnet_name': s['name']}))
                raise n_exc.OverlappingOfName(msg=err_msg)
    
        else:
            result = context.session.query(models_v2.Subnet).filter_by(tenant_id=tenant_id, name=s['name']).first()
            if result:
                err_msg = (_("The subnet '%(subnet_name)s' already exist") % {'subnet_name': s['name']})
                LOG.error(_LI(("The subnet '%(subnet_name)s' already exist") % {'subnet_name': s['name']}))
                raise n_exc.OverlappingOfName(msg=err_msg)
        #endif
