...
class OVSNeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def __init__(self, bridge_classes, conf=None):
        ...
        #begin:added by jimin3,bug-78246,2016-12-7
        self.duplicate_agent_times = 0
        self.quitting_rpc_timeout = agent_conf.quitting_rpc_timeout
        #end:added by jimin3,bug-78246,2016-12-7
        ...
        #begin:deleted by jimin3,bug-78246,2016-12-7
        # self.quitting_rpc_timeout = agent_conf.quitting_rpc_timeout
        #end: deleted by jimin3,bug-78246,2016-12-7
    ...
    def _report_state(self):
        ...
        try:
            agent_status = self.state_rpc.report_state(self.context,
                                                       ...
                                                       True)
            #begin:added by jimin3,bug-78246,2016-12-7
            if agent_status == n_const.AGENT_DUPLICATED:
                LOG.info(_LI('Duplicated agent,may need to exit'))
                self.duplicate_agent_times = self.duplicate_agent_times + 1
                if self.duplicate_agent_times > (cfg.CONF.agent_down_time / cfg.CONF.AGENT.report_interval + 1):
                    LOG.info(_LI('Self exit'))
    ...
    def check_ovs_status(self):
        #begin:modified by mall2, bug-75092, 2016-11-18
        full_args = ["ovs-ofctl", 'dump-flows', 'br-int', 'table=0']
        table0_flow = utils.execute(full_args, run_as_root=True, process_input=None)
        normal_flow = table0_flow.find('NORMAL')
        status = constants.OVS_NORMAL
        if normal_flow == -1:
            LOG.warning(_LW("OVS is restarted for NORMAL flow missed on br-int"))
            status = constants.OVS_RESTARTED
        else:
            # Check for the canary flow
            status = self.int_br.check_canary_table()
        #end:modified by mall2, bug-75092, 2016-11-18
        #begin:added by jimin3, bug-100654,2017-5-11
        if self.iter_num % 300 == 0:
            status = constants.OVS_RESTARTED
        #end:added by jimin3, THCI-100654,2017-5-11
