...
class RouterInfo(object):
    ...
    def _update_portforwardings(self, operation, portfwd):
        ...
        if operation == 'create':
            ...
            self.iptables_manager.ipv4['nat'].add_rule(chain_in, rule_in,
                                                       tag='portforwarding')
            LOG.debug("Added portforwarding rule_out is '%s'", rule_out)
            ...
            LOG.debug("Removed portforwarding rule_out is '%s'", rule_out)
    ...
    def _process_fip_bandwidth(self, ex_gw_port):
        try:
            if self.agent_conf.rate_limiting:
                self.setup_router_floating_ip_bandwidth()
            else:
                self.clear_router_bandwidth()
        except Exception as e:
            LOG.warn("Failed to set up rate limiting rules, %s", str(e))
            LOG.exception(e)

    def setup_router_floating_ip_bandwidth(self):
        """Configure floating ip rate limiting for router.
        """
        fips = self.router.get(l3_constants.FLOATINGIP_KEY, [])
        fips_with_gw = fips + [self.router['gw_floating_ip']]
        new_fips_gw_dict = dict([[fip["id"], fip] for fip in fips_with_gw])
        # Get removed and updated floating ips
        removed_fips = filter(
            lambda fip: self.floating_ips_gw_dict.get(fip, {}).
            get('floating_ip_address') != new_fips_gw_dict.get(fip, {}).
            get('floating_ip_address'), self.floating_ips_gw_dict)
        for fip_id in removed_fips:
            fip_address = self.floating_ips_gw_dict.get(fip_id).get('floating_ip_address')
            self.clear_floatingip_bandwidth(fip_address)

        ufips = filter(lambda fip: self.floating_ips_gw_dict.
                       get(fip['id'], {}).get('bandwidth') != fip['bandwidth'] or
                       self.floating_ips_gw_dict.get(fip['id'], {}).
                       get('floating_ip_address') != fip['floating_ip_address'], fips_with_gw)
        self.floating_ips_gw_dict = new_fips_gw_dict
        physical_nics = self.find_physical_nic()
        if physical_nics:
            for physical_nic in physical_nics:
                self.setup_egress_bandwidth(ufips, physical_nic, TC_EGRESS_ROOT_CLASS_ID)
                self.setup_ingress_bandwidth(ufips, physical_nic, TC_INGRESS_ROOT_CLASS_ID)

    def setup_ingress_bandwidth(self, fips, physical_nic, tc_ingress_root_class_id):
        LOG.info(_("setup_ingress_bandwidth"))
        ifb = self.find_ifb_nic()
        tc_lib.ensure_root_ingress_qdisc(physical_nic, ifb, tc_ingress_root_class_id)
        self.setup_fips_bandwidth(fips, ifb, tc_ingress_root_class_id, False)

    def setup_egress_bandwidth(self, fips, physical_nic, tc_egress_root_class_id):
        LOG.info(_("setup_egress_bandwidth"))
        tc_lib.ensure_root_htb_qdisc(physical_nic, tc_egress_root_class_id)
        self.setup_fips_bandwidth(fips, physical_nic, tc_egress_root_class_id, True)

    def setup_fips_bandwidth(self, fips, nic, root_class_id, is_egress=True):
        for fip in fips:
            bandwidth = fip['bandwidth']
            if bandwidth is None:
                bandwidth = self.agent_conf.default_north_south_bandwidth
            fip_address = fip['floating_ip_address']
            tc_lib.ensure_tc_class(nic, bandwidth, root_class_id)
            tc_lib.ensure_tc_filter(nic, bandwidth, root_class_id, fip_address, is_egress)

    def clear_floatingip_bandwidth(self, fip_address):
        if not fip_address:
            return
        physical_nics = self.find_physical_nic()
        if not physical_nics:
            return
        for physical_nic in physical_nics:
            tc_lib.clear_tc_filter(fip_address, physical_nic, TC_EGRESS_ROOT_CLASS_ID)
            tc_lib.clear_tc_filter(fip_address, self.find_ifb_nic(), TC_INGRESS_ROOT_CLASS_ID)

    def clear_router_floating_ip_bandwidth(self, removed_floating_ips, fip_info):
        for fip_id in removed_floating_ips:
            fip_address = fip_info.get(fip_id, [])
            self.clear_floatingip_bandwidth(fip_address)

    def clear_router_bandwidth(self):
        physical_nics = self.find_physical_nic()
        if physical_nics:
            for physical_nic in physical_nics:
                self.clear_egress_bandwidth(physical_nic)
                self.clear_ingress_bandwidth(physical_nic)

    def clear_egress_bandwidth(self, physical_nic):
        tc_lib.clear_egress(physical_nic, TC_EGRESS_ROOT_CLASS_ID)

    def clear_ingress_bandwidth(self, physical_nic):
        tc_lib.clear_ingress(physical_nic, self.find_ifb_nic(), TC_INGRESS_ROOT_CLASS_ID)

    def find_physical_nic(self):
        nics = self.agent_conf.external_physical_nic
        if nics:
            return [nic.strip() for nic in nics.split(',')]
        # assume we use ovs agent...
        nics = self._find_ovs_physical_nic(self.router.get('gw_provider_physical_network', None))
        if not nics:
            LOG.warn(_("Could not obtain external nic for public network. Thus"
                       " bandwidth won't be set"))
        return nics

    def _find_ovs_physical_nic(self, provider_network):
        bridge = None
        if provider_network:
            bridge_mappings = common_utils.parse_mappings(self.agent_conf.OVS.bridge_mappings)
            bridge = bridge_mappings.get(provider_network, None)
        if not bridge:
            bridge = self.agent_conf.external_network_bridge
        if bridge:
            return self._find_all_physical_nics(bridge)

    def _find_all_physical_nics(self, bridge):
        """Get physical nics through ovs bridges. The logic here is the
        physical interface(s) may be attached to external bridge 'br-ex'
        directly or through another ovs-bridge (e.g. br-ethX) and br-ethX
        is connected with br-ex by peer interfaces."""
        if not bridge:
            return
        ovs_ifaces = ovs_lib.OVSBridge(bridge).get_iface_name_list()
        base_ovs = ovs_lib.BaseOVS()
        nics = []
        for iface in ovs_ifaces:
            type = base_ovs.get_iface_type(iface)
            # type is '' or 'system' indicates a physical nic.
            # Refer to http://openvswitch.org/ovs-vswitchd.conf.db.5.pdf
            if (not type or type == 'system') and not iface.startswith('phy-'):
                nics.append(iface)
            if type == 'patch' and iface.startswith(bridge):
                peer_name = base_ovs.get_peer_interface(iface)
                br = base_ovs.get_bridge_for_iface(peer_name)
                br_ifaces = ovs_lib.OVSBridge(br).get_iface_name_list()
                for i in br_ifaces:
                    ti = base_ovs.get_iface_type(i)
                    if not ti or ti == 'system':
                        nics.append(i)
        return nics


    def find_ifb_nic(self):
        return "ifb0"
    ...
    def process_external(self, agent):
        ...
        try:
            ...
            # Setup bandwidth for floating IPs and gateway
            self._process_fip_bandwidth(ex_gw_port)
