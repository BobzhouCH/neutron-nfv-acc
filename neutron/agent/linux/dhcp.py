...
class Dnsmasq(DhcpLocalProcess):
    ...
    def _build_cmdline_callback(self, pid_file):
        ...
        for i, subnet in enumerate(self.network.subnets):
            ...
            if self.conf.dhcp_lease_duration == -1:
                ...
                #begin: modified by mall2, bug-75085, 2016-11-18
                random = int(self.network.id.split('-')[0], 16) % 1800
                lease = '%ss' % (self.conf.dhcp_lease_duration + random)
                #lease = '%ss' % self.conf.dhcp_lease_duration
                #end: modified by mall2, bug-75085, 2016-11-18
    ...
    def _output_init_lease_file(self):
        ...
        if self.conf.dhcp_lease_duration == -1:
            ...
            random = int(self.network.id.split('-')[0], 16) % 1800
            timestamp = int(time.time()) + self.conf.dhcp_lease_duration + random
            #end: modified by mall2, bug-75085, 2016-11-18
