...
class BaseOVS(object):
    ...
    def get_iface_type(self, iface_name, check_error=False, log_errors=True):
        return self.ovsdb.db_get('Interface', iface_name, 'type').execute(
            check_error=check_error, log_errors=log_errors)

    def get_peer_interface(self, iface_name, check_error=False, log_errors=True):
        return self.ovsdb.db_get('Interface', iface_name, 'options').execute(
            check_error=check_error, log_errors=log_errors).get('peer')
...
class OVSBridge(BaseOVS):
    ...
    def _update_bw_limit_queue(self, port_name, queue_uuid, queue_type,
                               other_config):
        if queue_uuid:
            self.set_db_attribute('Queue', queue_uuid, 'other_config',
                                  other_config, check_error=True)
        else:
            external_ids = {'id': port_name,
                            'queue_type': str(QOS_INGRESS_QUEUE)}
            self.ovsdb.db_create(
                'Queue', external_ids=external_ids,
                other_config=other_config).execute(check_error=True)
            queue_uuid = self._find_queue(port_name, QOS_INGRESS_QUEUE)
        return queue_uuid

    def _update_bw_limit_profile(self, port_name, qos_uuid,
                                 queue_uuid, queue_type):
        queues = {queue_type: queue_uuid}
        if qos_uuid:
            self.set_db_attribute(
                'QoS', qos_uuid,
                'queues', queues, check_error=True)
        else:
            queues = {queue_type: queue_uuid}
            external_ids = {'id': port_name}
            # The max-rate in other_config is the root max-rate.
            self.ovsdb.db_create(
                'QoS', external_ids=external_ids,
                other_config={'max-rate': 100000000000},
                type='linux-htb',
                queues=queues
            ).execute(check_error=True)
        qos_uuid = self._find_qos(port_name)
        return qos_uuid
    ...
    def update_ingress_bw_limit_for_port(self, port_name, max_kbps,
        ...
        queue_uuid = self._update_bw_limit_queue(
            port_name, queue_uuid, QOS_INGRESS_QUEUE, queue_other_config
        )

        qos_uuid = self._update_bw_limit_profile(
            port_name, qos_uuid, queue_uuid, QOS_INGRESS_QUEUE
        )

        self.set_db_attribute('Port', port_name, 'qos', qos_uuid,
                              check_error=True)
    ...
    def get_ingress_bw_limit_for_port(self, port_name):
        ...
        max_burst_kbit = int(other_config['burst']) / 1000
