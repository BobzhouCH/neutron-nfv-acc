...
class UnableCreateGwInternalPort(nexception.NeutronException):
    message = _("Unable to create the internal router gateway port which "
                "are used as a dummy internal port for floating ip of router"
                "gateway")

def _validate_bandwidth_or_none(data, valid_values=None):
    if data is not None:
        try:
            if attr.convert_to_int(data) < 0:
                raise nexception.InvalidInput()
        except nexception.InvalidInput:
            msg = _("'%s' is not a valid positive integer value for bandwidth") % data
            return msg
attr.validators['type:bandwidth'] = _validate_bandwidth_or_none
...
RESOURCE_ATTRIBUTE_MAP = {
    ...
    FLOATINGIPS: {
        ...
        'bandwidth': {'allow_post': True, 'allow_put': True,
                      'validate': {'type:bandwidth': None},
                    'is_visible': True, 'default': cfg.CONF.default_north_south_bandwidth},
    },
}
